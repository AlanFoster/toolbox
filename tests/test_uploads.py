...
def extract_token_id(response):
    assert response.status_code == HTTPStatus.OK

    match = re.search(
        'curl -F "file=@./test_upload_file.txt" -F "token_id=(.*)" http://localhost/uploads',
...
def test_upload_token_creation(app, client, req_ctx):
    ...
    assert response.status_code == HTTPStatus.OK
    assert extract_token_id(response) is not None
...
def test_upload_file(mocker: MockerFixture, app, client, req_ctx):
    ...
    token_id = extract_token_id(response)
